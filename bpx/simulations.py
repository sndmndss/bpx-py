...
def get_fees(filled_orders: list[dict]) -> float | int:
    ...
    fees = [
        (
            float(item["price"]) * float(item["fee"])
            if item["feeSymbol"] != "USDC"
            else float(item["fee"])
        )
...
def get_approximate_balance_in_usdc(
    balance: dict, time_interval: str = "1m", public: Public = default_public
) -> float:
    """
